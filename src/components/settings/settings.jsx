...
const SettingsForm = ({ image, username, bio, email, password, inProgress, onSubmitForm }) => {
  ...
  const updateState = field => ev => {
    setOState({ ...oState, [field]: ev.target.value });
  };
    ...
    <form onSubmit={submitForm}>
      <fieldset>
        <fieldset className='form-group'>
          <input className='form-control' type='text' placeholder='URL of profile picture' value={oState.image} onChange={updateState('image')} />
        </fieldset>
        ...
        <fieldset className='form-group'>
          <input className='form-control form-control-lg' type='email' placeholder='Email' value={oState.email} onChange={updateState('email')} />
        </fieldset>
...
const Settings = () => {
  ...
  const oCurrentUser = useSelector(store => ({ ...store.common.currentUser, ...store.settings }), shallowEqual);
  const onClickLogout = () => {
    dispatch({ type: LOGOUT });
  };

  const onSubmitForm = oUser => {
    dispatch({ type: SETTINGS_SAVED, payload: agent.Auth.save(oUser) });
  };
